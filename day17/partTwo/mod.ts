  ...
  while (heat_paths.length > 0) {
    ...
    for (
      ...
      if (
        ...
        // Don't add paths that will go out of bounds after 4 moves.
        if (
          next_dx == 1 && next_coordinates.x > grid[0].length - 4 ||
          next_dx == -1 && next_coordinates.x < 4 ||
          next_dy == 1 && next_coordinates.y > grid.length - 4 ||
          next_dy == -1 && next_coordinates.y < 4
        ) {
          continue;
        }
        if (inBounds(grid, next_coordinates)) {
